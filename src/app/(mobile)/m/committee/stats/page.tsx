...
export default function CommitteeStatsPage() {
  ...
  useEffect(() => {
    ...
    // 연도 전체 기준 monthly 데이터
    const yearStart = new Date().getFullYear();
    const monthlyParams = `from=${yearStart}-01-01&to=${yearStart}-12-31&granularity=month`;
  ...
  const kpis = overview ? [
    { label: '총 신청',   value: overview.totalRequests ?? 0,  color: 'text-purple-600', bg: 'bg-purple-50', icon: '📋' },
    { label: '승인 완료', value: overview.approvedRequests ?? 0, color: 'text-green-600',  bg: 'bg-green-50',  icon: '✅' },
    { label: '반려',      value: overview.rejectedRequests ?? 0, color: 'text-red-500',    bg: 'bg-red-50',    icon: '❌' },
    { label: '배차 완료', value: overview.totalDispatches ?? 0,  color: 'text-blue-600',   bg: 'bg-blue-50',   icon: '🚗' },
  ] : [];

  // 최근 6개월 bar 데이터
  const chartData = useMemo(() => {
    if (!monthly?.series) return [];
    return monthly.series.slice(-6).map((s: any) => ({
      name: s.label?.replace('월', '').slice(-2) + '월' || s.period,
      신청: s.requests ?? 0,
      배차: s.dispatches ?? 0,
    }));
  }, [monthly]);
      ...
      <div className="flex-1 px-4 py-4 space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
            ...
            <div className="flex items-center gap-3">
              <div className="flex-1 bg-gray-100 rounded-full h-3 overflow-hidden">
                <div
                  ...
                  style={{ width: `${Math.round((overview.approvedRequests / overview.totalRequests) * 100)}%` }}
                />
              </div>
            ...
            <div className="flex gap-4 mt-2">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[10px] text-gray-500">승인 {overview.approvedRequests}건</span>
              </div>
              <div className="flex items-center gap-1.5">
                <div className="w-2 h-2 rounded-full bg-red-400" />
                <span className="text-[10px] text-gray-500">반려 {overview.rejectedRequests}건</span>
              </div>
              <div className="flex items-center gap-1.5">
                <div className="w-2 h-2 rounded-full bg-gray-300" />
                <span className="text-[10px] text-gray-500">기타 {overview.totalRequests - overview.approvedRequests - overview.rejectedRequests}건</span>
              </div>
            </div>
          </div>
        )}
                  ...
                  <div className="flex items-center justify-between mb-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-medium text-gray-800 truncate max-w-[140px]">
                        {dept.departmentName || '미분류'}
                      </span>
                    </div>
                    <span className="text-sm font-bold text-gray-700">{dept.totalRequests}건</span>
                  </div>
                  <div className="bg-gray-100 rounded-full h-1.5 overflow-hidden">
                    <div
                      className="h-1.5 rounded-full transition-all duration-700"
                      style={{
                        width: `${Math.round((dept.totalRequests / maxDeptCount) * 100)}%`,
        ...
        {overview?.vehicleStats && (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                { label: '사용 가능', value: overview.vehicleStats.available ?? 0, color: 'text-green-600',  bg: 'bg-green-50' },
                { label: '운행 중',   value: overview.vehicleStats.inUse ?? 0,     color: 'text-blue-600',   bg: 'bg-blue-50' },
                { label: '정비 중',   value: overview.vehicleStats.maintenance ?? 0, color: 'text-orange-600', bg: 'bg-orange-50' },
                { label: '전체',      value: overview.vehicleStats.total ?? 0,      color: 'text-gray-700',   bg: 'bg-gray-50' },
              ].map(s => (
